            ...
            <div class="text-center mt-6">
                ...
                <div class="flex flex-wrap items-center justify-center gap-2 bg-zinc-100 dark:bg-zinc-800 border-t border-zinc-200 dark:border-zinc-700 -mx-6 -mb-6 p-6">
                    @if (Config::get('app.about_url') != '')
                        <flux:button size="sm" icon="external-link" target="_blank" :href="route('about')">{{ __('common.footer_about') }}</flux:button>
                    @endif
                    @if (Config::get('app.terms_url') != '')
                        <flux:button size="sm" icon="external-link" target="_blank" :href="route('terms')">{{ __('common.footer_terms') }}</flux:button>
                    @endif
                    @if (Config::get('app.privacy_url') != '')
                        <flux:button size="sm" icon="external-link" target="_blank" :href="route('privacy')">{{ __('common.footer_privacy') }}</flux:button>
                    @endif
