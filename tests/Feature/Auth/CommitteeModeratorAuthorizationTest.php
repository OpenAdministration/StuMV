...
/**
 * A committee moderator can act like a general moderator, but only for the
 * committee they were assigned to plus its descendants - see
 * App\Ldap\Committee::hasModerator() and the CommunityModerator middleware,
 * which resolves the {ou} route parameter (when present) into a committee-
 * scoped check instead of the old community-wide-only one.
 */
uses(RefreshDatabase::class);

test('a committee moderator can edit their own committee', function (): void {
...
test('a committee moderator can edit a descendant committee', function (): void {
    $community = newCommunity();
...
test('a committee moderator cannot edit an unrelated committee', function (): void {
    $community = newCommunity();
    ...
    $this->actingAs($moderator)
        ->get(route('committees.edit', ['uid' => $community->getShortCode(), 'ou' => 'committee-b']))
        ->assertStatus(403);
});

test('a committee moderator cannot edit their committee\'s own parent', function (): void {
    $community = newCommunity();
    $parent = TestLdap::makeCommittee($community, 'parent');
    $child = TestLdap::makeCommittee($community, 'child', parentDn: $parent->getDn());
    ...
    $this->actingAs($moderator)
        ->get(route('committees.edit', ['uid' => $community->getShortCode(), 'ou' => 'parent']))
        ->assertStatus(403);
});
