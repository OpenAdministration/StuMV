...
test('a membership without an end date is always active', function () {
    expect(membership(['from' => today()->subYear(), 'until' => null])->isActive())->toBeTrue();
});

test('a membership is active inside its window', function () {
    expect(membership(['from' => today()->subDay(), 'until' => today()->addDay()])->isActive())->toBeTrue();
});

test('a membership that already ended is not active', function () {
    expect(membership(['from' => today()->subWeek(), 'until' => today()->subDay()])->isActive())->toBeFalse();
});

test('a membership that has not started yet is not active', function () {
