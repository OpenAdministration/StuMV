 ...
 */
test('a community can be looked up by its uid', function () {
...
test('looking up an unknown community returns null', function () {
    expect(Community::findByUid('does-not-exist'))->toBeNull();
});

test('findOrFailByUid aborts with a 404 for an unknown community', function () {
    Community::findOrFailByUid('does-not-exist');
})->throws(NotFoundHttpException::class);

test('the members group exposes the community members', function () {
...
test('the admins group of the demo community contains its admin', function () {
...
test('a domain resolves back to the community that owns it', function () {
